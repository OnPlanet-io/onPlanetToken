...
contract onPlanet is Context, IERC20, Ownable {
    ...
    // For testing
    event feeCharged(address from, uint taxFee, uint buybackFee, uint teamFee);
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair) {
            ...
            if(multiFeeOn){
                // if last less time is more than a day, we update
                if(firstSellTime[from] + (1 days) < block.timestamp){
                    sellNumbers[from] = 0;
                }

                // if no selling in a day
        ...
        emit feeCharged(from, _taxFee, _buybackFee, _teamFee);
